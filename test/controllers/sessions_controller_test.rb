...
describe SessionsController do
  ...
  describe 'create' do
    describe 'when it is called with a known user' do
      before do
        post :create, email: users(:name1).email, password: 'Password1234'
      end
      it 'saves the user\'s id in the session' do
        session[:user_id].must_equal users(:name1).id
      end
    ...
    describe 'when it is called with the wrong password' do
      before do
        post :create, email: users(:name1).email, password: 'WrongPassword'
      end
      it 'returns to the sign in page' do
        assert_template :new
      end
